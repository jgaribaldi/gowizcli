...
func estimateLuminanceLux(
	...
	if sunElevationDeg > 0.0 {
		...
		return max(0.0, evLux)
	}

	if sunElevationDeg > TwilightNauticalLimitDeg {
		baseClear := twilightLux(sunElevationDeg)
		fh1 := math.Exp(-contextFactors.altitudeM / Fh1DenM)
		fh2 := math.Exp(-contextFactors.altitudeM / Fh2DenM)
		altitudeFactor := 1.0 - 0.5*(fh1-fh2)
		evTwilight := baseClear * kc * max(0.8, altitudeFactor)
		return max(0.0, evTwilight)
	}
...
const (
	TwilightCivilStartDeg      float64 = -6.0
	TwilightMaxLux             float64 = 200.0
	TwilightShapeK             float64 = 1.2
	TwilightNauticalLimitDeg   float64 = -12.0
	TwilightClearLuxAtNautical float64 = 1.0
)

func twilightLux(sunElevationDeg float64) float64 {
	if sunElevationDeg <= TwilightCivilStartDeg-4.0 {
		return 0.0
	}
